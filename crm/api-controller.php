...
if($searchValue != ''){
   $searchQuery = " and (title like '%".$searchValue."%' or
        ...
        custom_url like '%".$searchValue."%' or
...
$faq='<a href="javascript:PopupCenter(\'service-faqs.php?pid='.$row['id'].'\',\'Service FAQ\',900,600);" class="btn btn-success btn-xs"> <i class="fa fa-plus"></i></a>&nbsp; <a href="javascript:PopupCenter(\'service-faqs.php?pid='.$row['id'].'\',\'Service FAQ\',900,600);" class="btn btn-primary btn-xs"> <i class="fa fa-eye"></i></a>';

$last_update='<p class="text-danger">Created At: '.$c_at.'</p> <p class="text-success">Last Update At: '.$u_at.'</p>';
$sts=$row['status'] != 0 ? '<small class="label bg-green1">Active</small>': '<small class="label bg-red1">Inactive</small>';
$hpsts=$row['hp_status'] != 0 ? '<small class="label bg-green1">Show</small>': '<small class="label bg-red1">Hide</small>';
$charges=$row['charge'];
   $data[] = array(
      "id" => $row['id'],
      "sno" => $sn,
      "code" => $row['service_code'],
      "title"=>$row['title'].'<p><a href="https://www.techabilititsolutions.com/'.$row['custom_url'].'" target="_blank">'.$row['custom_url'].'</a></p>',
      ...
      "medias"=> '<p class="text-danger">Medias: '.$medias.'</p><p class="text-success">FAQ: '.$faq.'</p>',
      "last_update" => $last_update,
      "status" => $sts."<br/>".$hpsts,
      "actions" => $varact
   );
...
## Total number of records without filtering
$sel = mysqli_query($obj->con,"select count(*) as allcount from brands");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];
## Total number of record with filtering
$sel = mysqli_query($obj->con,"select count(*) as allcount from brands WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];
## Fetch records
...
while ($row = mysqli_fetch_assoc($empRecords)) {
// $subcategory='<a href="subcategories.php?cid='.$row['id'].'" class="btn btn-success btn-xs"> <i class="fa fa-plus"></i></a>&nbsp; <a href="subcategories.php?cid='.$row['id'].'&m=active" class="btn btn-primary btn-xs"> <i class="fa fa-eye"></i></a>';
$varact=  '<a href="brands.php?id='.$row['id'].'" class="btn btn-primary btn-xs"> <i class="fa fa-edit"></i></a>&nbsp;<a href="#!" onclick="javascript:deleteThisRecord('.$row['id'].',\'brands\');" class="btn btn-danger btn-xs"> <i class="fa fa-trash-o"></i></a>';
$c_at= date_format(date_create($row['created_at']),'d-M-Y h:i A');
$u_at= $row['updated_at'] != null ? date_format(date_create($row['updated_at']),'d-M-Y h:i A') : $c_at;
...
## Total number of records without filtering
$sel = mysqli_query($obj->con,"select count(*) as allcount from gallery");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];
## Total number of record with filtering
$sel = mysqli_query($obj->con,"select count(*) as allcount from gallery WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];
## Fetch records
 $empQuery = "select * from gallery WHERE 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
$empRecords = mysqli_query($obj->con, $empQuery);
$data = array();
$sn=1;
while ($row = mysqli_fetch_assoc($empRecords)) {
// $subcategory='<a href="subcategories.php?cid='.$row['id'].'" class="btn btn-success btn-xs"> <i class="fa fa-plus"></i></a>&nbsp; <a href="subcategories.php?cid='.$row['id'].'&m=active" class="btn btn-primary btn-xs"> <i class="fa fa-eye"></i></a>';
$varact=  '<a href="gallery.php?id='.$row['id'].'" class="btn btn-primary btn-xs"> <i class="fa fa-edit"></i></a>&nbsp;<a href="#!" onclick="javascript:deleteThisRecord('.$row['id'].',\'gallery\');" class="btn btn-danger btn-xs"> <i class="fa fa-trash-o"></i></a>';
$c_at= date_format(date_create($row['created_at']),'d-M-Y h:i A');
$u_at= $row['updated_at'] != null ? date_format(date_create($row['updated_at']),'d-M-Y h:i A') : $c_at;
 $attachments='<p class="text-danger"> <a href="../'.$row['file_url'].'" style="cursor:pointer;" class="btn btn-primary btn-xs example-image-link" data-lightbox="example-1" ><i class="fa fa-eye" ></i></a>';
$last_update='<p class="text-danger">Created At: '.$c_at.'</p> <p class="text-success">Last Update At: '.$u_at.'</p>';
