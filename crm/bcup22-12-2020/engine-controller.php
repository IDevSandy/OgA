...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if( $obj -> insert_record('services',$myArray)) {
     ...
     if($last_id != "") {
       ...
       $NArray=array(
         "service_code" => 'TA/Services/'.date("y").'/'.sprintf("%04d", $last_id)
       );
...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if( $obj -> insert_record('laboratories',$myArray)) {
     ...
     if($last_id != "") {
       $where= array("id"=> $last_id);
       $NArray=array(
         "labcode" => 'TA/Labs/'.date("y").'/'.sprintf("%04d", $last_id)
       );
...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if( $obj -> insert_record('vendors',$myArray)) {
     ...
     if($last_id != "") {
       $where= array("id"=> $last_id);
       $NArray=array(
         "vendor_code" => 'TA/Vendor/'.date("y").'/'.sprintf("%04d", $last_id)
       );
...
//  header("location:clients.php?type=error&msg=Error in Updating Record!");
  ...
  if( $obj -> insert_record('clients',$myArray)) {
     ...
     if($last_id != "") {
       $where= array("id"=> $last_id);
       $NArray=array(
         "client_code" => 'TA/Clients/'.date("y").'/'.sprintf("%04d", $last_id)
       );
...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if($obj-> update_record("leads",$where,$myArray)) {
    if($target_file != ''){
      $myNArray= array(
        "uuid" => $uuid,
        "title"=> 'Proposal for TA/Leads/'.date("y").'/'.sprintf("%04d", $_REQUEST['id']),
        "client_id"=> mysqli_real_escape_string($obj->con,$_POST['client_id']),
        "remarks"=> 'Proposal for TA/Leads/'.date("y").'/'.sprintf("%04d", $_REQUEST['id']),
        "status" => 1,
        "file_url" => $target_file,
      "created_at" => $datetime
      );
  ...
  if( $obj -> insert_record('leads',$myArray)) {
     ...
     if($last_id != "") {
       $where= array("id"=> $last_id);
       $NArray=array(
         "lead_code" => 'TA/Leads/'.date("y").'/'.sprintf("%04d", $last_id)
       );
      ...
      if($target_file != ''){
        $myNArray= array(
          "uuid" => $uuid,
          "title"=> 'Proposal for TA/Leads/'.date("y").'/'.sprintf("%04d", $last_id),
          "client_id"=> mysqli_real_escape_string($obj->con,$_POST['client_id']),
          "remarks"=> 'Proposal for TA/Leads/'.date("y").'/'.sprintf("%04d", $last_id),
          "status" => 1,
          "file_url" => $target_file,
        "created_at" => $datetime
        );
...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if( $obj -> insert_record('invoices',$myArray)) {
     ...
     if($last_id != "") {
       $where= array("id"=> $last_id);
       $NArray=array(
         "invoice_code" => 'TA/Inv/'.date("y").'/'.sprintf("%04d", $last_id)
       );
...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if( $obj -> insert_record('projects',$myArray)) {
     ...
     if($last_id != "") {
       $where= array("id"=> $last_id);
       $NArray=array(
         "project_code" => 'TA/Projects/'.date("y").'/'.sprintf("%04d", $last_id)
       );
...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if($obj-> update_record("expenses",$where,$myArray)) {
      $myNArray= array(
         "expense_code" => 'TA/Expenses/'.date("y").'/'.sprintf("%04d", $_REQUEST['id']),
        "payment_date"=> mysqli_real_escape_string($obj->con,$_POST['payment_date']),
        "mode" => mysqli_real_escape_string($obj->con,$_POST['mode']),
        "send_by" => $_SESSION['USER_ID'],
        "credited_to" => mysqli_real_escape_string($obj->con,$_POST['credited_to']),
  ...
  if( $obj -> insert_record('expenses',$myArray)) {
     ...
     if($last_id != "") {
       $where= array("id"=> $last_id);
       $NArray=array(
         "expense_code" => 'TA/Expenses/'.date("y").'/'.sprintf("%04d", $last_id)
       );
...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if( $obj -> insert_record('zones',$myArray)) {
    ...
    if($last_id != "") {
      $where= array("id"=> $last_id);
      $NArray=array(
        "zone_code" => 'TA/Zones/'.date("y").'/'.sprintf("%04d", $last_id)
      );
...
if(isset($_POST['id']) && $_POST['id'] != "") {
  ...
  header("location:service-faqs.php?type=success&msg=Record Updated Successfully!");
} else {
  header("location:service-faqs.php?type=error&msg=Error in Updating Record!");
}
} else {
  $myArray= array(
    "uuid" => $uuid,
  ...
  if( $obj -> insert_record('service_faqs',$myArray)) {
    header("location:service-faqs.php?type=success&msg=Record Saved Successfully!");
  } else {
    header("location:service-faqs.php?type=error&msg=Error in Saving Record!");
  }
}
break;
/* end service Faq*/
...
if(isset($_POST['id']) && $_POST['id'] != "") {
  $myArray= array(
    ...
    "permalink"=> mysqli_real_escape_string($obj->con,$_POST['permalink']),
...
if(isset($_REQUEST['id']) && $_REQUEST['id'] != "") {
  ...
  if( $obj -> insert_record('employees',$myArray)) {
    ...
    if($last_id != "") {
      $where= array("id"=> $last_id);
      $NArray=array(
        "emp_code" => 'TA/Employees/'.date("y").'/'.sprintf("%04d", $last_id)
      );
      $obj->update_record("employees",$where,$NArray);
