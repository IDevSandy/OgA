function RecordView() {
  ...
  destroyTable();
    ...
    $.ajax({
        ...
        data: {
            action: 'get_records',tbl:'subcategories'
        },
         async: true,
         dataType: "json",
         success: function(data) {
            ...
            if (data.RESPONSE_CODE == '2XX') {
                ...
                for (i = 0; i < allData.length; i++) {
                    ...
                    dynHTML += '<tr class="gradeX"><td>'+id+'</td><td>' + cttl + '</td><td>' + pttl + '</td><td>' + sts + '</td><td><a href="subcategories.php?id=' + id + '"><i class="material-icons">edit</i></a></td><td><a href="javascript:void(0);" class="cs-delete" onclick="javascript:deleteRow(' + id + ',\'subcategories\')" ><i class="material-icons">delete_sweep</i></a></td></tr>';
                }
            } else {
            emptyRecord();
            }
...
function sendData() {
    if (document.getElementsByName('title')[0].value != "" && document.getElementsByName('category_id')[0].value != "" && document.getElementsByName('status')[0].value != "") {
        $.ajax({
            type: 'post',
            cache: false,
            url: 'api-controller.php',
